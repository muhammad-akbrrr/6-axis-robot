...
def setCom():
    try:
        ...
        almStatusLab.config(
            text="UNABLE TO ESTABLISH COMMUNICATIONS WITH TEENSY 4.1 CONTROLLER", style="Alarm.TLabel")
        almStatusLab2.config(
            text="UNABLE TO ESTABLISH COMMUNICATIONS WITH TEENSY 4.1 CONTROLLER", style="Alarm.TLabel")
        Curtime = datetime.datetime.now().strftime("%B %d %Y - %I:%M%p")
        tab8.ElogView.insert(
            END, Curtime+" - UNABLE TO ESTABLISH COMMUNICATIONS WITH TEENSY 4.1 CONTROLLER")
        value = tab8.ElogView.get(0, END)
        pickle.dump(value, open("ErrorLog", "wb"))
...
def darkTheme():
    ...
    style = ttk.Style()
    style.configure("Alarm.TLabel", foreground="IndianRed1",
                    font=('Arial', '10', 'bold'))
    style.configure("Warn.TLabel", foreground="orange",
                    font=('Arial', '10', 'bold'))
    style.configure("OK.TLabel", foreground="light green",
                    font=('Arial', '10', 'bold'))
    style.configure("Jointlim.TLabel", foreground="light blue",
                    font=('Arial', '8'))
...
def lightTheme():
    ...
    style = ttk.Style()
    style.configure("Alarm.TLabel", foreground="red",
                    font=('Arial', '10', 'bold'))
    style.configure("Warn.TLabel", foreground="dark orange",
                    font=('Arial', '10', 'bold'))
    style.configure("OK.TLabel", foreground="green",
                    font=('Arial', '10', 'bold'))
    style.configure("Jointlim.TLabel", foreground="dark blue",
                    font=('Arial', '8'))
        ...
        while tab1.runTrue == 1:
            if (tab1.runTrue == 0):
                if (estopActive == TRUE):
                    ...
                    almStatusLab.config(
                        text="Position Out of Reach",  style="Alarm.TLabel")
                    almStatusLab2.config(
                        text="Position Out of Reach",  style="Alarm.TLabel")
                else:
                    almStatusLab.config(
                        text="PROGRAM STOPPED",  style="Alarm.TLabel")
                    almStatusLab2.config(
                        text="PROGRAM STOPPED",  style="Alarm.TLabel")
            else:
                almStatusLab.config(text="PROGRAM RUNNING",  style="OK.TLabel")
                almStatusLab2.config(
                    text="PROGRAM RUNNING",  style="OK.TLabel")
            rowinproc = 1
            executeRow()
            while rowinproc == 1:
                time.sleep(.1)
...
def stopProg():
    ...
    tab1.runTrue = 0
    if (estopActive == TRUE):
        almStatusLab.config(text="Estop Button was Pressed",
                            style="Alarm.TLabel")
        almStatusLab2.config(
            text="Estop Button was Pressed",  style="Alarm.TLabel")
    elif (posOutreach == TRUE):
        almStatusLab.config(text="Position Out of Reach",
                            style="Alarm.TLabel")
        almStatusLab2.config(text="Position Out of Reach",
                             style="Alarm.TLabel")
    else:
...
def executeRow():
    ...
    if (cmdType == "Read C"):
        ...
        try:
            ...
            Curtime = datetime.datetime.now().strftime("%B %d %Y - %I:%M%p")
            tab8.ElogView.insert(
                END, Curtime+" - UNABLE TO ESTABLISH COMMUNICATIONS WITH SERIAL DEVICE")
            value = tab8.ElogView.get(0, END)
            pickle.dump(value, open("ErrorLog", "wb"))
        ser3.flushInput()
    ...
    if (cmdType == "TifOn "):
        ...
        if (response == "T"):
            tabNum = ("Tab Number " +
                      str(command[tabIndex+4:])).encode('utf-8')
            index = tab1.progView.get(0, "end").index(tabNum)
            index = index-1
            tab1.progView.selection_clear(0, END)
            tab1.progView.select_set(index)
    ...
    if (cmdType == "Move J"):
        ...
        LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
            str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
            str(J6OpenLoopStat.get())
        command = "MJ"+"X"+xVal+"Y"+yVal+"Z"+zVal+"Rz"+rzVal+"Ry"+ryVal+"Rx"+rxVal+"J7"+J7Val+"J8" + \
            J8Val+"J9"+J9Val+speedPrefix+Speed+"Ac"+ACCspd + \
            "Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
        cmdSentEntryField.delete(0, 'end')
        cmdSentEntryField.insert(0, command)
        ser.write(command.encode())
        ser.flushInput()
    ...
    if (cmdType == "Move V"):
        ...
        LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
            str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
            str(J6OpenLoopStat.get())
        command = "MV"+"X"+xVal+"Y"+yVal+"Z"+zVal+"Rz"+rzVal+"Ry"+ryVal+"Rx"+rxVal+"J7"+J7Val+"J8"+J8Val+"J9" + \
            J9Val+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd + \
            "Rm"+ACCramp+"W"+WC+"Vr"+visRot+"Lm"+LoopMode+"\n"
        cmdSentEntryField.delete(0, 'end')
        cmdSentEntryField.insert(0, command)
        ser.write(command.encode())
        ser.flushInput()
    ...
    if (cmdType == "OFF PR"):
        ...
        xVal = str(float(eval("SP_"+SP+"_E1_EntryField").get()) +
                   float(eval("SP_"+SP2+"_E1_EntryField").get()))
        yVal = str(float(eval("SP_"+SP+"_E2_EntryField").get()) +
                   float(eval("SP_"+SP2+"_E2_EntryField").get()))
        zVal = str(float(eval("SP_"+SP+"_E3_EntryField").get()) +
                   float(eval("SP_"+SP2+"_E3_EntryField").get()))
        rzVal = str(float(eval("SP_"+SP+"_E4_EntryField").get()) +
                    float(eval("SP_"+SP2+"_E4_EntryField").get()))
        ryVal = str(float(eval("SP_"+SP+"_E5_EntryField").get()) +
                    float(eval("SP_"+SP2+"_E5_EntryField").get()))
        rxVal = str(float(eval("SP_"+SP+"_E6_EntryField").get()) +
    ...
    if (cmdType == "Move L"):
        ...
        command = "ML"+"X"+xVal+"Y"+yVal+"Z"+zVal+"Rz"+rzVal+"Ry"+ryVal+"Rx"+rxVal+"J7"+J7Val+"J8"+J8Val+"J9"+J9Val + \
            speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp + \
            "Rnd"+Rounding+"W"+WC+"Lm"+LoopMode+"Q"+DisWrist+"\n"
    ...
    if (cmdType == "Move R"):
        ...
        LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
            str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
            str(J6OpenLoopStat.get())
        command = "RJ"+"A"+J1Val+"B"+J2Val+"C"+J3Val+"D"+J4Val+"E"+J5Val+"F"+J6Val+"J7"+J7Val+"J8" + \
            J8Val+"J9"+J9Val+speedPrefix+Speed+"Ac"+ACCspd + \
            "Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
        cmdSentEntryField.delete(0, 'end')
        cmdSentEntryField.insert(0, command)
    ...
    if (cmdType == "Move A"):
        ...
        if (subCmd == "Move A End"):
            almStatusLab.config(
                text="Move A must start with a Mid followed by End", style="Alarm.TLabel")
            almStatusLab2.config(
                text="Move A must start with a Mid followed by End", style="Alarm.TLabel")
        else:
            xIndex = command.find(" X ")
            ...
            LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
                str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
                str(J6OpenLoopStat.get())
            command = "MA"+"X"+xVal+"Y"+yVal+"Z"+zVal+"Rz"+rzVal+"Ry"+ryVal+"Rx"+rxVal+"Ex"+Xend+"Ey"+Yend+"Ez" + \
                Zend+"Tr"+trVal+speedPrefix+Speed+"Ac"+ACCspd+"Dc" + \
                DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
            cmdSentEntryField.delete(0, 'end')
            cmdSentEntryField.insert(0, command)
            ser.write(command.encode())
            ser.flushInput()
    ...
    if (cmdType == "Move C"):
        ...
        if (subCmd == "Move C Sta" or subCmd == "Move C Pla"):
            almStatusLab.config(
                text="Move C must start with a Center followed by Start & Plane", style="Alarm.TLabel")
            almStatusLab2.config(
                text="Move C must start with a Center followed by Start & Plane", style="Alarm.TLabel")
        else:
            xIndex = command.find(" X ")
            ...
            LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
                str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
                str(J6OpenLoopStat.get())
            command = "MJ"+"X"+Xmid+"Y"+Ymid+"Z"+Zmid+"Rz"+rzVal+"Ry"+ryVal+"Rx"+rxVal+"Tr" + \
                trVal+speedPrefix+Speed+"Ac"+ACCspd+"Dc" + \
                DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
            ser.write(command.encode())
            ser.flushInput()
            time.sleep(.1)
            ...
            LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
                str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
                str(J6OpenLoopStat.get())
            command = "MC"+"Cx"+xVal+"Cy"+yVal+"Cz"+zVal+"Rz"+rzVal+"Ry"+ryVal+"Rx"+rxVal+"Bx"+Xmid+"By"+Ymid+"Bz"+Zmid+"Px" + \
                Xend+"Py"+Yend+"Pz"+Zend+"Tr"+trVal+speedPrefix+Speed+"Ac" + \
                ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
            cmdSentEntryField.delete(0, 'end')
            cmdSentEntryField.insert(0, command)
            ser.write(command.encode())
            ser.flushInput()
    ...
    if (cmdType == "Vis Fi"):
        ...
        if (status == "pass"):
            tabNum = (
                "Tab Number " + str(command[passIndex+6:failIndex]) + "\r\n").encode('utf-8')
            index = tab1.progView.get(0, "end").index(tabNum)
            tab1.progView.selection_clear(0, END)
            tab1.progView.select_set(index)
        elif (status == "fail"):
            tabNum = ("Tab Number " +
                      str(command[failIndex+6:]) + "\r\n").encode('utf-8')
            index = tab1.progView.get(0, "end").index(tabNum)
            tab1.progView.selection_clear(0, END)
            tab1.progView.select_set(index)
...
def xbox():
    def threadxbox():
        ...
        if xboxUse == 0:
            ...
            jogMode = 1
            grip = 0
            almStatusLab.config(text='JOGGING JOINTS 1 & 2',
                                style="Warn.TLabel")
            almStatusLab2.config(
            ...
            xboxUse = 0
            almStatusLab.config(text='XBOX CONTROLLER OFF',
                                style="Warn.TLabel")
            almStatusLab2.config(
                text='XBOX CONTROLLER OFF', style="Warn.TLabel")
            ...
            try:
                ...
                for event in events:
                    ...
                    # J7 MODE
                        ...
                        mainMode = 4
                        almStatusLab.config(
                            text='JOGGING TRACK', style="Warn.TLabel")
                        almStatusLab2.config(
                            text='JOGGING TRACK', style="Warn.TLabel")
                    # TRACK JOG
...
def J1jogNeg(value):
    ...
    if (speedtype == "mm per Sec"):
        speedMenu = OptionMenu(tab1, speedOption, "Percent",
                               "Percent", "Seconds", "mm per Sec")
        speedPrefix = "Sp"
        speedEntryField.delete(0, 'end')
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "RJ"+"A"+str(float(J1AngCur)-value)+"B"+J2AngCur+"C"+J3AngCur+"D"+J4AngCur+"E"+J5AngCur+"F"+J6AngCur+"J7"+str(
        J7PosCur)+"J8"+str(J8PosCur)+"J9"+str(J9PosCur)+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    ser.write(command.encode())
    ser.flushInput()
...
def J2jogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "RJ"+"A"+J1AngCur+"B"+str(float(J2AngCur)-value)+"C"+J3AngCur+"D"+J4AngCur+"E"+J5AngCur+"F"+J6AngCur+"J7"+str(
        J7PosCur)+"J8"+str(J8PosCur)+"J9"+str(J9PosCur)+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    ser.write(command.encode())
...
def J3jogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "RJ"+"A"+J1AngCur+"B"+J2AngCur+"C"+str(float(J3AngCur)-value)+"D"+J4AngCur+"E"+J5AngCur+"F"+J6AngCur+"J7"+str(
        J7PosCur)+"J8"+str(J8PosCur)+"J9"+str(J9PosCur)+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    ser.write(command.encode())
...
def J4jogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "RJ"+"A"+J1AngCur+"B"+J2AngCur+"C"+J3AngCur+"D"+str(float(J4AngCur)-value)+"E"+J5AngCur+"F"+J6AngCur+"J7"+str(
        J7PosCur)+"J8"+str(J8PosCur)+"J9"+str(J9PosCur)+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    ser.write(command.encode())
...
def J5jogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "RJ"+"A"+J1AngCur+"B"+J2AngCur+"C"+J3AngCur+"D"+J4AngCur+"E"+str(float(J5AngCur)-value)+"F"+J6AngCur+"J7"+str(
        J7PosCur)+"J8"+str(J8PosCur)+"J9"+str(J9PosCur)+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    ser.write(command.encode())
...
def J6jogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "RJ"+"A"+J1AngCur+"B"+J2AngCur+"C"+J3AngCur+"D"+J4AngCur+"E"+J5AngCur+"F"+str(float(J6AngCur)-value)+"J7"+str(
        J7PosCur)+"J8"+str(J8PosCur)+"J9"+str(J9PosCur)+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def J7jogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "RJ"+"A"+J1AngCur+"B"+J2AngCur+"C"+J3AngCur+"D"+J4AngCur+"E"+J5AngCur+"F"+J6AngCur+"J7" + \
        str(float(J7PosCur)-value)+"J8"+str(J8PosCur)+"J9"+str(J9PosCur)+speedPrefix + \
        Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    ser.flushInput()
...
def J8jogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "RJ"+"A"+J1AngCur+"B"+J2AngCur+"C"+J3AngCur+"D"+J4AngCur+"E"+J5AngCur+"F"+J6AngCur+"J7"+str(J7PosCur)+"J8"+str(
        float(J8PosCur)-value)+"J9"+str(J9PosCur)+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    ser.flushInput()
...
def J9jogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        ...
        str(J6OpenLoopStat.get())
    command = "RJ"+"A"+J1AngCur+"B"+J2AngCur+"C"+J3AngCur+"D"+J4AngCur+"E"+J5AngCur+"F"+J6AngCur+"J7"+str(J7PosCur)+"J8"+str(
        J8PosCur)+"J9"+str(float(J9PosCur)-value)+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    ser.flushInput()
...
def LiveJointJog(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "LJ"+"V"+str(value)+speedPrefix+Speed+"Ac" + \
        ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def LiveCarJog(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "LC"+"V"+str(value)+speedPrefix+Speed+"Ac" + \
        ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def LiveToolJog(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "LT"+"V"+str(value)+speedPrefix+Speed+"Ac" + \
        ACCspd+"Dc"+DECspd+"Rm"+ACCramp+"W"+WC+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TXjogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTX1"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    cmdSentEntryField.delete(0, 'end')
...
def TYjogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTY1"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TZjogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTZ1"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TRxjogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTW1"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TRyjogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTP1"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TRzjogNeg(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTR1"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TXjogPos(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTX0"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TYjogPos(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTY0"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TZjogPos(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTZ0"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TRxjogPos(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTW0"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TRyjogPos(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTP0"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def TRzjogPos(value):
    ...
    LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
        str(J4OpenLoopStat.get())+str(J5OpenLoopStat.get()) + \
        str(J6OpenLoopStat.get())
    command = "JTR0"+str(value)+speedPrefix+Speed+"G" + \
        ACCspd+"H"+DECspd+"I"+ACCramp+"Lm"+LoopMode+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def teachInsertBelSelected():
    ...
    if (movetype == "OFF J"):
        ...
        newPos = movetype + " [*] X "+XcurPos+" Y "+YcurPos+" Z "+ZcurPos+" Rz "+RzcurPos+" Ry "+RycurPos+" Rx "+RxcurPos+" J7 "+str(
            J7PosCur)+" J8 "+str(J8PosCur)+" J9 "+str(J9PosCur)+" "+speedPrefix+" "+Speed+" Ac "+ACCspd + " Dc "+DECspd+" Rm "+ACCramp+" $ "+WC
    ...
    if (movetype == "Move Vis"):
        ...
        movetype = movetype+" [ PR: "+str(SavePosEntryField.get())+" ]"
        newPos = movetype + " [*]"+" J7 "+str(J7PosCur)+" J8 "+str(J8PosCur)+" J9 "+str(
            J9PosCur)+" "+speedPrefix+" "+Speed+" Ac "+ACCspd + " Dc "+DECspd+" Rm "+ACCramp+" $ "+WC
        tab1.progView.insert(selRow, bytes(newPos + '\n', 'utf-8'))
        tab1.progView.selection_clear(0, END)
        tab1.progView.select_set(selRow)
        items = tab1.progView.get(0, END)
        ...
        movetype = movetype+" [ PR: "+str(SavePosEntryField.get()) + \
            " ] offs [ *PR: "+str(int(SavePosEntryField.get())+1)+" ] "
        newPos = movetype + " [*]"+" J7 "+str(J7PosCur)+" J8 "+str(J8PosCur)+" J9 "+str(
            J9PosCur)+" "+speedPrefix+" "+Speed+" Ac "+ACCspd + " Dc "+DECspd+" Rm "+ACCramp+" $ "+WC
        tab1.progView.insert(selRow, bytes(newPos + '\n', 'utf-8'))
        tab1.progView.selection_clear(0, END)
        ...
        newPos = movetype + " [*] X "+XcurPos+" Y "+YcurPos+" Z "+ZcurPos+" Rz "+RzcurPos+" Ry "+RycurPos+" Rx "+RxcurPos+" J7 "+str(J7PosCur)+" J8 "+str(
            J8PosCur)+" J9 "+str(J9PosCur)+" "+speedPrefix+" "+Speed+" Ac "+ACCspd + " Dc "+DECspd+" Rm "+ACCramp+" Rnd "+Rounding+" $ "+WC
        tab1.progView.insert(selRow, bytes(newPos + '\n', 'utf-8'))
        tab1.progView.selection_clear(0, END)
        tab1.progView.select_set(selRow)
        items = tab1.progView.get(0, END)
        ...
        newPos = movetype + " [*] J1 "+J1AngCur+" J2 "+J2AngCur+" J3 "+J3AngCur+" J4 "+J4AngCur+" J5 "+J5AngCur+" J6 "+J6AngCur+" J7 "+str(
            J7PosCur)+" J8 "+str(J8PosCur)+" J9 "+str(J9PosCur)+" "+speedPrefix+" "+Speed+" Ac "+ACCspd + " Dc "+DECspd+" Rm "+ACCramp+" $ "+WC
        tab1.progView.insert(selRow, bytes(newPos + '\n', 'utf-8'))
        tab1.progView.selection_clear(0, END)
        tab1.progView.select_set(selRow)
...
def loadProg():
    ...
    filename = fd.askopenfilename(
        title='Open files', initialdir=folder, filetypes=filetypes)
...
def CreateProg():
    user_input = simpledialog.askstring(
        title="New Program", prompt="New Program Name:")
...
def insertvisFind():
    ...
    value = "Vis Find - "+template+" - BGcolor "+BGcolor + \
        " Score "+score+" Pass "+passTab+" Fail "+failTab
    tab1.progView.insert(selRow, bytes(value + '\n', 'utf-8'))
    tab1.progView.selection_clear(0, END)
    tab1.progView.select_set(selRow)
    items = tab1.progView.get(0, END)
...
def calRobotAll():
    ##### STAGE 1 ########
    command = "LL"+"A"+str(J1CalStatVal)+"B"+str(J2CalStatVal)+"C"+str(J3CalStatVal)+"D"+str(J4CalStatVal)+"E"+str(J5CalStatVal)+"F"+str(J6CalStatVal)+"G"+str(J7CalStatVal)+"H"+str(J8CalStatVal) + \
        "I"+str(J9CalStatVal)+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(J4calOff) + \
        "N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff) + \
        "Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
    ser.flushInput()
    ...
    pickle.dump(value, open("ErrorLog", "wb"))
    ##### STAGE 2 ########
    CalStatVal2 = int(J1CalStatVal2)+int(J2CalStatVal2)+int(J3CalStatVal2) + \
        int(J4CalStatVal2)+int(J5CalStatVal2)+int(J6CalStatVal2)
    if (CalStatVal2 > 0):
        command = "LL"+"A"+str(J1CalStatVal2)+"B"+str(J2CalStatVal2)+"C"+str(J3CalStatVal2)+"D"+str(J4CalStatVal2)+"E"+str(J5CalStatVal2)+"F"+str(J6CalStatVal2)+"G"+str(J7CalStatVal2)+"H"+str(
            J8CalStatVal2)+"I"+str(J9CalStatVal2)+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(J4calOff)+"N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff)+"Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
        ser.write(command.encode())
        cmdSentEntryField.delete(0, 'end')
        cmdSentEntryField.insert(0, command)
        ser.flushInput()
...
def calRobotJ1():
    command = "LLA1B0C0D0E0F0G0H0I0"+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(
...
def calRobotJ2():
    command = "LLA0B1C0D0E0F0G0H0I0"+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(
        J4calOff)+"N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff)+"Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def calRobotJ3():
    command = "LLA0B0C1D0E0F0G0H0I0"+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(
        J4calOff)+"N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff)+"Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def calRobotJ4():
    command = "LLA0B0C0D1E0F0G0H0I0"+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(
        J4calOff)+"N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff)+"Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def calRobotJ5():
    command = "LLA0B0C0D0E1F0G0H0I0"+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(
        J4calOff)+"N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff)+"Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def calRobotJ6():
    command = "LLA0B0C0D0E0F1G0H0I0"+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(
        J4calOff)+"N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff)+"Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def calRobotJ7():
    command = "LLA0B0C0D0E0F0G1H0I0"+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(
        J4calOff)+"N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff)+"Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def calRobotJ8():
    command = "LLA0B0C0D0E0F0G0H1I0"+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(
        J4calOff)+"N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff)+"Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def calRobotJ9():
    command = "LLA0B0C0D0E0F0G0H0I1"+"J"+str(J1calOff)+"K"+str(J2calOff)+"L"+str(J3calOff)+"M"+str(
        J4calOff)+"N"+str(J5calOff)+"O"+str(J6calOff)+"P"+str(J7calOff)+"Q"+str(J8calOff)+"R"+str(J9calOff)+"\n"
    ser.write(command.encode())
    cmdSentEntryField.delete(0, 'end')
    cmdSentEntryField.insert(0, command)
...
def updateParams():
    ...
    J1EncMult = str(float(J1EncCPREntryField.get()) /
                    float(J1DriveMSEntryField.get()))
    J2EncMult = str(float(J2EncCPREntryField.get()) /
                    float(J2DriveMSEntryField.get()))
    J3EncMult = str(float(J3EncCPREntryField.get()) /
                    float(J3DriveMSEntryField.get()))
    J4EncMult = str(float(J4EncCPREntryField.get()) /
                    float(J4DriveMSEntryField.get()))
    J5EncMult = str(float(J5EncCPREntryField.get()) /
                    float(J5DriveMSEntryField.get()))
    J6EncMult = str(float(J6EncCPREntryField.get()) /
                    float(J6DriveMSEntryField.get()))
    ...
    J1jogslide.config(from_=float("-"+J1NegLim), to=float(J1PosLim),
                      length=180, orient=HORIZONTAL,  command=J1sliderUpdate)
    J2negLimLab.config(text="-"+J2NegLim, style="Jointlim.TLabel")
    J2posLimLab.config(text=J2PosLim, style="Jointlim.TLabel")
    J2jogslide.config(from_=float("-"+J2NegLim), to=float(J2PosLim),
                      length=180, orient=HORIZONTAL,  command=J2sliderUpdate)
    J3negLimLab.config(text="-"+J3NegLim, style="Jointlim.TLabel")
    J3posLimLab.config(text=J3PosLim, style="Jointlim.TLabel")
    J3jogslide.config(from_=float("-"+J3NegLim), to=float(J3PosLim),
                      length=180, orient=HORIZONTAL,  command=J3sliderUpdate)
    J4negLimLab.config(text="-"+J4NegLim, style="Jointlim.TLabel")
    J4posLimLab.config(text=J4PosLim, style="Jointlim.TLabel")
    J4jogslide.config(from_=float("-"+J4NegLim), to=float(J4PosLim),
                      length=180, orient=HORIZONTAL,  command=J4sliderUpdate)
    J5negLimLab.config(text="-"+J5NegLim, style="Jointlim.TLabel")
    J5posLimLab.config(text=J5PosLim, style="Jointlim.TLabel")
    J5jogslide.config(from_=float("-"+J5NegLim), to=float(J5PosLim),
                      length=180, orient=HORIZONTAL,  command=J5sliderUpdate)
    J6negLimLab.config(text="-"+J6NegLim, style="Jointlim.TLabel")
    J6posLimLab.config(text=J6PosLim, style="Jointlim.TLabel")
    J6jogslide.config(from_=float("-"+J6NegLim), to=float(J6PosLim),
                      length=180, orient=HORIZONTAL,  command=J6sliderUpdate)
...
def calExtAxis():
    ...
    J7jogslide.config(from_=-J7NegLim, to=J7PosLim,  length=125,
                      orient=HORIZONTAL,  command=J7sliderUpdate)
    J8jogslide.config(from_=-J8NegLim, to=J8PosLim,  length=125,
                      orient=HORIZONTAL,  command=J8sliderUpdate)
    J9jogslide.config(from_=-J9NegLim, to=J9PosLim,  length=125,
                      orient=HORIZONTAL,  command=J9sliderUpdate)

    command = "CE"+"A"+str(J7PosLim)+"B"+str(J7rotation)+"C"+str(J7steps)+"D"+str(J8PosLim)+"E"+str(
        J8rotation)+"F"+str(J8steps)+"G"+str(J9PosLim)+"H"+str(J9rotation)+"I"+str(J9steps)+"\n"
    ser.write(command.encode())
    ser.flushInput()
    time.sleep(.1)
    response = ser.read()
...
def zeroAxis7():
    ...
    time.sleep(.1)
    almStatusLab.config(
        text="J7 Calibration Forced to Zero", style="Warn.TLabel")
    almStatusLab2.config(
        text="J7 Calibration Forced to Zero", style="Warn.TLabel")
...
def zeroAxis8():
    ...
    time.sleep(.1)
    almStatusLab.config(
        text="J8 Calibration Forced to Zero", style="Warn.TLabel")
    almStatusLab2.config(
        text="J8 Calibration Forced to Zero", style="Warn.TLabel")
...
def zeroAxis9():
    ...
    time.sleep(.1)
    almStatusLab.config(
        text="J9 Calibration Forced to Zero", style="Warn.TLabel")
    almStatusLab2.config(
        text="J9 Calibration Forced to Zero", style="Warn.TLabel")
...
def sendPos():
    command = "SP"+"A"+str(J1AngCur)+"B"+str(J2AngCur)+"C"+str(J3AngCur)+"D"+str(J4AngCur)+"E"+str(
        J5AngCur)+"F"+str(J6AngCur)+"G"+str(J7PosCur)+"H"+str(J8PosCur)+"I"+str(J9PosCur)+"\n"
    ser.write(command.encode())
    ser.flushInput()
    time.sleep(.1)
    response = ser.read()
...
def CalZeroPos():
    ...
    almStatusLab2.config(
        text="Calibration Forced to Home", style="Warn.TLabel")
...
def take_pic():
    ...
    autoBGVal = int(autoBG.get())
    if (autoBGVal == 1):
        BG1 = cv2image[int(VisX1PixEntryField.get())
                       ][int(VisY1PixEntryField.get())]
        BG2 = cv2image[int(VisX1PixEntryField.get())
                       ][int(VisY2PixEntryField.get())]
        BG3 = cv2image[int(VisX2PixEntryField.get())
                       ][int(VisY2PixEntryField.get())]
...
def mask_crop(event, x, y, flags, param):
    ...
    # if the left mouse button was released
        ...
        autoBGVal = int(autoBG.get())
        if (autoBGVal == 1):
            BG1 = oriImage[int(VisX1PixEntryField.get())
                           ][int(VisY1PixEntryField.get())]
            BG2 = oriImage[int(VisX1PixEntryField.get())
                           ][int(VisY2PixEntryField.get())]
            BG3 = oriImage[int(VisX2PixEntryField.get())
                           ][int(VisY2PixEntryField.get())]
            avg = int(mean([BG1, BG2, BG3]))
            BGavg = (avg, avg, avg)
            background = avg
...
def rotate_image(img, angle, background):
    ...
    result = cv2.warpAffine(
        img, rot_mat, img.shape[1::-1], borderMode=cv2.BORDER_CONSTANT, borderValue=background, flags=cv2.INTER_LINEAR)
    return result
...
def updateVisOp():
    ...
    if getattr(sys, 'frozen', False):
        ...
        folder = os.path.dirname(os.path.realpath(__file__))
    # folder = os.path.dirname(os.path.realpath(__file__))
    filelist = [fname for fname in os.listdir(
        folder) if fname.endswith('.jpg')]
...
def gcodeFrame():
    ...
    tab7.gcodeView = Listbox(gcodeframe, width=105,
                             height=46, yscrollcommand=scrollbar.set)
...
def loadGcodeProg():
    filetypes = (('gcode files', '*.gcode *.nc *.ngc *.cnc *.tap'),
                 ('text files', '*.txt'))
    filename = fd.askopenfilename(
...
def MoveGcodeStartPos():
    xVal = str(float(GC_ST_E1_EntryField.get()) +
               float(GC_SToff_E1_EntryField.get()))
    yVal = str(float(GC_ST_E2_EntryField.get()) +
               float(GC_SToff_E2_EntryField.get()))
    zVal = str(float(GC_ST_E3_EntryField.get()) +
               float(GC_SToff_E3_EntryField.get()))
    rzVal = str(float(GC_ST_E4_EntryField.get()) +
                float(GC_SToff_E4_EntryField.get()))
    ryVal = str(float(GC_ST_E5_EntryField.get()) +
                float(GC_SToff_E5_EntryField.get()))
    rxVal = str(float(GC_ST_E6_EntryField.get()) +
                float(GC_SToff_E6_EntryField.get()))
    J7Val = str(J7PosCur)
    J8Val = str(J8PosCur)
    J9Val = str(J9PosCur)
    speedPrefix = "Sm"
    ...
    if (response[:1] == 'E'):
        ...
        if (status == "yes"):
            GCalmStatusLab.config(
                text="FILES FOUND ON SD CARD:",  style="OK.TLabel")
        GcodeProgEntryField.delete(0, 'end')
        tab7.gcodeView.delete(0, END)
...
def GCstopProg():
    ...
    lastProg = ""
    tab7.GCrunTrue = 0
    GCalmStatusLab.config(text="GCODE CONVERSION STOPPED",
                          style="Alarm.TLabel")
    if (splineActive == 1):
        splineActive = "0"
        if (stopQueue == "1"):
    ...
    if (cmdType == "G"):
        ...
        if (subCmd == "90" or subCmd == "91" or subCmd == "28"):
            ...
            LoopMode = str(J1OpenLoopStat.get())+str(J2OpenLoopStat.get())+str(J3OpenLoopStat.get()) + \
                ...
                str(J6OpenLoopStat.get())
            Filename = GcodeFilenameField.get() + ".txt"
            command = "WC"+"X"+xVal+"Y"+yVal+"Z"+zVal+"Rz"+rzVal+"Ry"+ryVal+"Rx"+rxVal+"J7"+J7Val+"J8"+J8Val+"J9" + \
                J9Val+speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm" + \
                ACCramp+"W"+WC+"Lm"+LoopMode+"Fn"+Filename+"\n"
            cmdSentEntryField.delete(0, 'end')
            ...
            if (response[:1] == 'E'):
                ...
                tab7.GCrunTrue = 0
                GCalmStatusLab.config(
                    text="UNABLE TO WRITE TO SD CARD",  style="Alarm.TLabel")
            else:
                displayPosition(response)
        ...
        if (subCmd == "0" or subCmd == "1"):
            ...
            if (xVal != ""):
                ...
                xVal = str(
                    round((float(GC_ST_E1_EntryField.get())+float(xVal)), 3))
            else:
                try:
                    if (prevxVal != 0):
                        xVal = prevxVal
            ...
            if (yVal != ""):
                ...
                yVal = str(
                    round((float(GC_ST_E2_EntryField.get())+float(yVal)), 3))
            else:
                try:
                    if (prevyVal != 0):
                        yVal = prevyVal
            ...
            if (zVal != ""):
                ...
                zVal = str(
                    round((float(GC_ST_E3_EntryField.get())+float(zVal)), 3))
            else:
                try:
                    if (prevzVal != 0):
                        zVal = prevzVal
            ...
            if (bVal != ""):
                ryVal = str(
                    round((float(GC_ST_E5_EntryField.get())+float(bVal))), 3)
            else:
                ryVal = str(RycurPos)

            if (cVal != ""):
                rxVal = str(
                    round((float(GC_ST_E6_EntryField.get())+float(cVal)), 3))
            else:
                rxVal = str(RxcurPos)
            ...
            command = "WC"+"X"+xVal+"Y"+yVal+"Z"+zVal+"Rz"+rzVal+"Ry"+ryVal+"Rx"+rxVal+"J7"+J7Val+"J8"+J8Val+"J9"+J9Val + \
                speedPrefix+Speed+"Ac"+ACCspd+"Dc"+DECspd+"Rm"+ACCramp + \
                "Rnd"+Rounding+"W"+WC+"Lm"+LoopMode+"Fn"+Filename+"\n"
            prevxVal = xVal
            prevyVal = yVal
            prevzVal = zVal
            cmdSentEntryField.delete(0, 'end')
...
almStatusLab = Label(
    tab1, text="SYSTEM READY - NO ACTIVE ALARMS", style="OK.TLabel")
...
J1negLimLab = Label(J1jogFrame, font=("Arial", 8),
                    text=str(-J1NegLim), style="Jointlim.TLabel")
J1negLimLab.place(x=115, y=25)
J1posLimLab = Label(J1jogFrame, font=("Arial", 8),
                    text=str(J1PosLim), style="Jointlim.TLabel")
...
J1jogslide = Scale(J1jogFrame, from_=-J1NegLim, to=J1PosLim,
                   length=180, orient=HORIZONTAL,  command=J1sliderUpdate)
...
J2negLimLab = Label(J2jogFrame, font=("Arial", 8),
                    text=str(-J2NegLim), style="Jointlim.TLabel")
J2negLimLab.place(x=115, y=25)
J2posLimLab = Label(J2jogFrame, font=("Arial", 8),
                    text=str(J2PosLim), style="Jointlim.TLabel")
...
J2jogslide = Scale(J2jogFrame, from_=-J2NegLim, to=J2PosLim,
                   length=180, orient=HORIZONTAL,  command=J2sliderUpdate)
...
J3negLimLab = Label(J3jogFrame, font=("Arial", 8),
                    text=str(-J3NegLim), style="Jointlim.TLabel")
J3negLimLab.place(x=115, y=25)
J3posLimLab = Label(J3jogFrame, font=("Arial", 8),
                    text=str(J3PosLim), style="Jointlim.TLabel")
...
J3jogslide = Scale(J3jogFrame, from_=-J3NegLim, to=J3PosLim,
                   length=180, orient=HORIZONTAL,  command=J3sliderUpdate)
...
J4negLimLab = Label(J4jogFrame, font=("Arial", 8),
                    text=str(-J4NegLim), style="Jointlim.TLabel")
J4negLimLab.place(x=115, y=25)
J4posLimLab = Label(J4jogFrame, font=("Arial", 8),
                    text=str(J4PosLim), style="Jointlim.TLabel")
...
J4jogslide = Scale(J4jogFrame, from_=-J4NegLim, to=J4PosLim,
                   length=180, orient=HORIZONTAL,  command=J4sliderUpdate)
...
J5negLimLab = Label(J5jogFrame, font=("Arial", 8),
                    text=str(-J5NegLim), style="Jointlim.TLabel")
J5negLimLab.place(x=115, y=25)
J5posLimLab = Label(J5jogFrame, font=("Arial", 8),
                    text=str(J5PosLim), style="Jointlim.TLabel")
...
J5jogslide = Scale(J5jogFrame, from_=-J5NegLim, to=J5PosLim,
                   length=180, orient=HORIZONTAL,  command=J5sliderUpdate)
...
J6negLimLab = Label(J6jogFrame, font=("Arial", 8),
                    text=str(-J6NegLim), style="Jointlim.TLabel")
J6negLimLab.place(x=115, y=25)
J6posLimLab = Label(J6jogFrame, font=("Arial", 8),
                    text=str(J6PosLim), style="Jointlim.TLabel")
...
J6jogslide = Scale(J6jogFrame, from_=-J6NegLim, to=J6PosLim,
                   length=180, orient=HORIZONTAL,  command=J6sliderUpdate)
...
J7negLimLab = Label(J7jogFrame, font=("Arial", 8),
                    text=str(-J7NegLim), style="Jointlim.TLabel")
J7negLimLab.place(x=10, y=30)
J7posLimLab = Label(J7jogFrame, font=("Arial", 8),
                    text=str(J7PosLim), style="Jointlim.TLabel")
...
J7jogslide = Scale(J7jogFrame, from_=-J7NegLim, to=J7PosLim,
                   length=125, orient=HORIZONTAL,  command=J7sliderUpdate)
...
J8negLimLab = Label(J8jogFrame, font=("Arial", 8),
                    text=str(-J8NegLim), style="Jointlim.TLabel")
J8negLimLab.place(x=10, y=30)
J8posLimLab = Label(J8jogFrame, font=("Arial", 8),
                    text=str(J8PosLim), style="Jointlim.TLabel")
...
J8jogslide = Scale(J8jogFrame, from_=-J8NegLim, to=J8PosLim,
                   length=125, orient=HORIZONTAL,  command=J8sliderUpdate)
...
J9negLimLab = Label(J9jogFrame, font=("Arial", 8),
                    text=str(-J9NegLim), style="Jointlim.TLabel")
J9negLimLab.place(x=10, y=30)
J9posLimLab = Label(J9jogFrame, font=("Arial", 8),
                    text=str(J9PosLim), style="Jointlim.TLabel")
...
J9jogslide = Scale(J9jogFrame, from_=-J9NegLim, to=J9PosLim,
                   length=125, orient=HORIZONTAL,  command=J9sliderUpdate)
...
scrollbar.pack(side=RIGHT, fill=Y)
tab1.progView = Listbox(progframe, exportselection=0,
                        width=105, height=31, yscrollcommand=scrollbar.set)
...
menu = OptionMenu(tab1, options, "Move J", "Move J", "OFF J", "Move L", "Move R", "Move A Mid", "Move A End", "Move C Center",
                  "Move C Start", "Move C Plane", "Start Spline", "End Spline", "Move PR", "OFF PR ", "Teach PR", "Move Vis", command=posRegFieldVisible)
...
teachInsBut = Button(tab1,  text="Teach New Position",
                     width=22, command=teachInsertBelSelected)
teachInsBut.place(x=700, y=220)

teachReplaceBut = Button(tab1, text="Modify Position",
                         width=22,  command=teachReplaceSelected)
...
CalibrateBut = Button(tab1,  text="Auto Calibrate CMD",
                      width=22,   command=insCalibrate)
...
waitTimeBut = Button(tab1, text="Wait Time (seconds)",
                     width=22,  command=waitTime)
waitTimeBut.place(x=700, y=460)

waitInputOnBut = Button(tab1, text="Wait Input ON",
                        width=22,  command=waitInputOn)
waitInputOnBut.place(x=700, y=500)

waitInputOffBut = Button(tab1,  text="Wait Input OFF",
                         width=22,  command=waitInputOff)
waitInputOffBut.place(x=700, y=540)

setOutputOnBut = Button(tab1,  text="Set Output On",
                        width=22,  command=setOutputOn)
setOutputOnBut.place(x=700, y=580)

setOutputOffBut = Button(tab1,  text="Set Output OFF",
                         width=22,   command=setOutputOff)
...
insertIfCMDBut = Button(tab1,  text="Insert IF CMD",
                        width=15,   command=IfCMDInsert)
...
readAuxComBut = Button(tab1,  text="Read COM Device",
                       width=22,   command=ReadAuxCom)
...
StorPosBut = Button(tab1,  text="Position Register",
                    width=22,   command=storPos)
StorPosBut.place(x=950, y=560)

callBut = Button(tab1,  text="Call Program",
                 width=22,   command=insertCallProg)
callBut.place(x=950, y=600)

returnBut = Button(tab1,  text="Return",  width=22,   command=insertReturn)
returnBut.place(x=950, y=640)

visFindBut = Button(tab1,  text="Vision Find",
                    width=22,   command=insertvisFind)
...
storPosEqLab = Label(tab1, font=("Arial", 6),
                     text=" Pos Reg      Element       (++/--)")
...
almStatusLab2 = Label(
    tab2, text="SYSTEM READY - NO ACTIVE ALARMS", style="OK.TLabel")
...
axis7pinsetLab = Label(tab2, font=("Arial", 8),
                       text="StepPin = 12 / DirPin = 13 / CalPin = 36")
axis7pinsetLab.place(x=627, y=510)

axis8pinsetLab = Label(tab2, font=("Arial", 8),
                       text="StepPin = 32 / DirPin = 33 / CalPin = 37")
axis8pinsetLab.place(x=827, y=510)

axis9pinsetLab = Label(tab2, font=("Arial", 8),
                       text="StepPin = 34 / DirPin = 35 / CalPin = 38")
...
J7zerobut = Button(tab2, text="Set Axis 7 Calibration to Zero",
                   width=28, command=zeroAxis7)
J7zerobut.place(x=627, y=440)

J8zerobut = Button(tab2, text="Set Axis 8 Calibration to Zero",
                   width=28, command=zeroAxis8)
J8zerobut.place(x=827, y=440)

J9zerobut = Button(tab2, text="Set Axis 9 Calibration to Zero",
                   width=28, command=zeroAxis9)
J9zerobut.place(x=1027, y=440)

J7calbut = Button(tab2, text="Autocalibrate Axis 7",
                  width=28, command=calRobotJ7)
J7calbut.place(x=627, y=475)

J8calbut = Button(tab2, text="Autocalibrate Axis 8",
                  width=28, command=calRobotJ8)
J8calbut.place(x=827, y=475)

J9calbut = Button(tab2, text="Autocalibrate Axis 9",
                  width=28, command=calRobotJ9)
...
CalZeroBut = Button(tab2,   text="Force CaL to Home",
                    width=20,   command=CalZeroPos)
CalZeroBut.place(x=270, y=485)

CalRestBut = Button(tab2,   text="Force Cal to Rest",
                    width=20,   command=CalRestPos)
...
saveCalBut = Button(tab2,  text="    SAVE    ",  width=26,
...
DisableWristCbut = Checkbutton(
    tab3, text="Disable Wrist Rotation - Linear Moves", variable=DisableWristRot)
...
loadAR4Mk2But = Button(tab3,  text="Load AR4-MK3 Defaults",
                       width=26, command=LoadAR4Mk3default)
loadAR4Mk2But.place(x=1150, y=470)

loadAR4Mk2But = Button(tab3,  text="Load AR4-MK2 Defaults",
                       width=26, command=LoadAR4Mk2default)
loadAR4Mk2But.place(x=1150, y=510)

loadAR4But = Button(tab3,  text="Load AR4 Defaults",
                    width=26, command=LoadAR4default)
loadAR4But.place(x=1150, y=550)

loadAR3But = Button(tab3,  text="Load AR3 Defaults",
                    width=26, command=LoadAR3default)
loadAR3But.place(x=1150, y=590)


#### SAVE ####

saveCalBut = Button(tab3,  text="SAVE",  width=26,
                    command=SaveAndApplyCalibration)
saveCalBut.place(x=1150, y=630)
...
inoutavailLab = Label(
    tab4, text="NOTE: the following are available when using the default Nano board for IO:   Inputs = 2-7  /  Outputs = 8-13  /  Servos = A0-A7")
inoutavailLab.place(x=10, y=640)

inoutavailLab = Label(
    tab4, text="If using IO on Teensy board:  Inputs = 32-36  /  Outputs = 37-41 - if using IO on Teensy you must manually change the command from 'Out On =' to 'ToutOn ='")
...
TeachImgBut = Button(tab6,  text="Teach Object",
                     width=15, command=selectTemplate)
...
VisBrightSlide = Scale(tab6, from_=-127, to=127,
...
VisContrastSlide = Scale(tab6, from_=-127, to=127,
                         length=250, orient=HORIZONTAL)
...
pickClosestCbut = Checkbutton(
    tab6, text="Try Closest When Out of Range", variable=pickClosest)
pickClosestCbut.place(x=900, y=295)


saveCalBut = Button(tab6,  text="SAVE VISION DATA",
                    width=26, command=SaveAndApplyCalibration)
...
bgAutoCbut = Checkbutton(tab6, command=checkAutoBG,
                         text="Auto", variable=autoBG)
...
tab7.gcodeView = Listbox(gcodeframe, exportselection=0,
                         width=105, height=43, yscrollcommand=gcodescrollbar.set)
tab7.gcodeView.bind('<<ListboxSelect>>', gcodeViewselect)
tab7.gcodeView.pack()
gcodescrollbar.config(command=tab7.gcodeView.yview)
...
LoadGcodeBut = Button(tab7,  text="Load Program",
                      width=25, command=loadGcodeProg)
LoadGcodeBut.place(x=20, y=20)

GcodeStartPosBut = Button(tab7,  text="Set Start Position",
                          width=25, command=SetGcodeStartPos)
GcodeStartPosBut.place(x=20, y=100)

GcodeMoveStartPosBut = Button(
    tab7,  text="Move to Start Offset", width=25, command=MoveGcodeStartPos)
GcodeMoveStartPosBut.place(x=20, y=240)

runGcodeBut = Button(tab7, text="Convert & Upload to SD",
                     width=25,   command=GCconvertProg)
# playGPhoto=PhotoImage(file="play-icon.gif")
# runGcodeBut.config(image=playGPhoto)
runGcodeBut.place(x=20, y=375)

stopGcodeBut = Button(tab7, text="Stop Conversion & Upload",
                      width=25,  command=GCstopProg)
# stopGPhoto=PhotoImage(file="stop-icon.gif")
# stopGcodeBut.config(image=stopGPhoto)
stopGcodeBut.place(x=190, y=375)

delGcodeBut = Button(tab7, text="Delete File from SD",
                     width=25,   command=GCdelete)
delGcodeBut.place(x=20, y=415)

readGcodeBut = Button(tab7, text="Read Files from SD",
                      width=25,   command=partial(GCread, "yes"))
readGcodeBut.place(x=20, y=455)

playGPhoto = PhotoImage(file="play-icon.gif")
readGcodeBut = Button(tab7, text="Play Gcode File", width=20,
                      command=GCplay, image=playGPhoto, compound=LEFT)
...
saveGCBut = Button(tab7,  text="SAVE DATA",  width=26,
                   command=SaveAndApplyCalibration)
...
scrollbar.pack(side=RIGHT, fill=Y)
tab8.ElogView = Listbox(Elogframe, width=150, height=40,
...
link = Label(tab9, font='12',
             text="https://www.anninrobotics.com/tutorials",  cursor="hand2")
...
configfile = Text(tab9, wrap=WORD, width=166,
                  height=40, yscrollcommand=scroll.set)
...
msg = "Copyright © 2024 by Exe Eyes. All Rights Reserved"

tkinter.messagebox.showwarning("Copyright notice", msg)
xboxUse = 0
